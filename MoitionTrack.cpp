...
#if 0
int CMotionTracker::optTrack_OLD(cv::Mat prevFrame, cv::Mat curFrame, Point2d &centroid, cv::Rect &BBox, std::vector<cv::Point2d> &directions)
{
	std::vector<cv::Point2f> points[2];
	cv::Point2d dir_1; //DIRECTIONS OF THE MOVEMENT OF THE TRACKED OBJECTS
	int temp_idx = 0;
	...
	Size  winSize(18, 18), build_winSize(30, 30);
	Mat frameDisp;

	goodFeaturesToTrack(prevFrame, points[0], MAX_FEATURES, m_params.quality_level, m_params.block_size, Mat(), 3, m_params.use_harris, m_params.harris_k);
	if (points[0].size() == 0)
		int debug = 10;
	else {
		calcOpticalFlowPyrLK(prevFrame, curFrame, points[0], points[1], status, err, winSize, 3, m_params.termcrit, 0, 0.001);
		temp_idx = 0;
		for (idx = 0; idx < (int)points[1].size(); idx++)
		{
			if (status[idx] == 0)
				continue;
			float  debugf_dist = distance(points[1][idx], points[0][idx]);
			if (debugf_dist < MIN_MOTION_DISTANCE || debugf_dist > 60)//if (debugf_dist < MIN_MOTION_DISTANCE)
			{
				status[idx] = 0;
			}
			else {
				m_tp_point_x[temp_idx] = points[1][idx].x + BBox.x;
				m_tp_point_y[temp_idx] = points[1][idx].y + BBox.y;
				dir_1.x = distance(points[1][idx], points[0][idx]); //SIZE OF THE MOVEMENT	
				dir_1.y = std::atan2((double)(points[1][idx].y - points[0][idx].y), (double)(points[1][idx].x - points[0][idx].x)); //DIRECTION OF THE MOVEMENT
...
#if 0
	if (ELAD_DEBUG_MODE && flag1) {
		l = 0;
		for (idx = 0; idx < temp_idx; idx++) {

			CvPoint p;
			p = cvPoint((int)points[1][idx].x, (int)points[1][idx].y);
			l++;
			cv::circle(curFrame, p, 4, cvScalar(255, 0, 0), 1);
		}
		namedWindow("TRACK", WINDOW_NORMAL);
		resizeWindow("TRACK", 400, 400);
		moveWindow("TRACK", 800, 200);
		imshow("TRACK", curFrame);
		waitKey(1);
	...
	/*if (temp_idx < 15)
		return 0;*/
		//CALC CENTROID
	findCentroid(m_tp_point_x, m_tp_point_y, temp_idx, centroid);
	/*if (centroid.x < 10)
		int cloom = 0;*/
	if (temp_idx > 4) {
		BBox.x = ((centroid.x - (WIDTH / 2)) > 1) ? (centroid.x - (WIDTH / 2)) : 1;
		BBox.y = ((centroid.y - (HEIGHT / 2)) > 1) ? (centroid.y - (HEIGHT / 2)) : 1;
		BBox.width = WIDTH;
		BBox.height = HEIGHT;
	}
